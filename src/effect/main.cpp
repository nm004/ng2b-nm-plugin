...
init ()
{
...

  {
    ...
    patch2 = new Patch {0x18dc0b8, uint32_t {0x02000B}};
    //patch3 = new Patch {, uint16_t {0x1002}};
    //patch4 = new Patch {, uint16_t {0x1002}};

    // These make the enemy dead state detection delayed.
    // jmp 0xa0
    patch6 = new Patch {0x0f9eec7, make_bytes (0xe9, 0xa0, 0x00, 0x00, 0x00)};
    // xor eax, eax
    patch7 = new Patch {0x0f9ef6c, make_bytes (0x31, 0xc0)};
  }
  {
    using namespace bloodparticle;
    // These invoke large blood particles effect (each for humans and fiends).
    patch1 = new Patch {0x12b84bc + 1, uint8_t {0x47}};
    patch2 = new Patch {0x12b845d + 1, uint8_t {0x2a}};
    ...
    //
    patch3 = new Patch {0x12b3a9f + 1, 0x5};
    patch4 = new Patch {0x12b3aad + 1, 0x3};
    *reinterpret_cast <uintptr_t *> (base_address + 0x1d97348) = base_address + 0x12b3a50;
    //*reinterpret_cast <uintptr_t *> (base_address + 0x1c8e1f8) = base_address + 0x1205880;
    //patch3 = new Patch {0x12b3cdc + 1, 0x12b3bce - (0x12b3cdc + 5)};
    //patch3 = new Patch {0x12b3328 + 1, 0x12b3bce - (0x12b3328 + 5)};
    //patch4 = new Patch {0x12b3bce, make_bytes (0xB2, 0x05)};
  }

  {
    ...
    //patch7 = new Patch {0x12bb0ea, concat (NOP7, uint8_t {0xeb})};
    //patch8 = new Patch {0x12bacf3, concat (NOP7, uint8_t {0xeb})};

    //BloodStampEffectManager_vfunction2_hook = new VFPHook {0x197e940, BloodStampEffectManager_vfunction2};
    //SphericalResourceGenerator_vfunction2_hook = new VFPHook {0x19b8198, SphericalResourceGenerator_vfunction2};
  }

  {
    ...
    // Credits: Fiend Busa
    // This will trigger hitstop (micro freeeze) when a player dismembers an enemy's limb.
    // In Tag Missions, this is disabled. We want it always to be disabled.
    // jmp imm;
    patch1 = new Patch {0x15289de, uint8_t {0xeb}};
    ...
    //patch2 = new Patch {0x145866a + 1, 0x1220060 - (0x145866a + 5)};
    ...
    patch3 = new Patch {0x12cfbe7 + 1, 0x12b5760 - (0x12cfbe7 + 5)};
    // We also modify the internal jump table to make larger blood jet always.
    patch4 = new Patch {0x12b59f0, *reinterpret_cast <int32_t (*)[4]> (base_address + 0x12b59f0 + 4 * 0x10)};
